...
class Hnice_Woocommerce_Extra {
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'hnice') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'hnice')); ?>
            ...
            <?php
            if ($quantity > 0) { ?>
                <span class="deal-sold-counts"><strong> <?php echo wp_kses_data(sprintf(_n('%s product is sold', '%s products are sold', $abc, 'hnice'), $sales_counts)) ?></strong></span>';
            <?php }
            ?>
        </p>
    ...
    public function quantity_input_args($args, $product) {
        if (!hnice_woocommerce_is_deal_product($product)) {
            return $args;
        }
