...
class Hnice_breadcrumb {
	...
	private function __construct() {
		...
		add_filter('hnice_page_title', [$this, 'hide_title']);
	...
	public function breadcrumb_register() {
		$labels = [
			'name'               => esc_html__('Breadcrumbs', 'hnice'),
			'singular_name'      => esc_html__('Breadcrumb', 'hnice'),
			'menu_name'          => esc_html__('Breadcrumbs', 'hnice'),
			'name_admin_bar'     => esc_html__('Breadcrumb', 'hnice'),
			'add_new'            => esc_html__('Add New', 'hnice'),
			'add_new_item'       => esc_html__('Add New Breadcrumb', 'hnice'),
			'new_item'           => esc_html__('New Header Footer & Blocks Template', 'hnice'),
			'edit_item'          => esc_html__('Edit Header Footer & Blocks Template', 'hnice'),
			'view_item'          => esc_html__('View Header Footer & Blocks Template', 'hnice'),
			'all_items'          => esc_html__('All Breadcrumb', 'hnice'),
			'search_items'       => esc_html__('Search Header Footer & Blocks Templates', 'hnice'),
			'parent_item_colon'  => esc_html__('Parent Header Footer & Blocks Templates:', 'hnice'),
			'not_found'          => esc_html__('No Header Footer & Blocks Templates found.', 'hnice'),
			'not_found_in_trash' => esc_html__('No Header Footer & Blocks Templates found in Trash.', 'hnice'),
		];
		...
		register_post_type('hnice-breadcrumb', $args);
	}
	...
	public function register_admin_menu() {
		add_submenu_page(
			'themes.php',
			esc_html__('Breadcrumbs', 'hnice'),
			esc_html__('Breadcrumbs', 'hnice'),
			'edit_pages',
			'edit.php?post_type=hnice-breadcrumb'
		);
	}
	...
	function ehf_register_metabox() {
		add_meta_box(
			'ehf-meta-box',
			esc_html__('Breadcrumb Options', 'hnice'),
			[
				$this,
				'efh_metabox_render',
			],
		...
		<table class="hfe-options-table widefat">
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('Display On', 'hnice'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
					   title="<?php echo esc_attr__('Add locations for where this template should appear.', 'hnice'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					...
					HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
						...
						[
							'title'          => esc_html__('Display Rules', 'hnice'),
							'value'          => '[{"type":"basic-global","specific":null}]',
							'tags'           => 'site,enable,target,pages',
							'rule_type'      => 'display',
							'add_rule_label' => esc_html__('Add Display Rule', 'hnice'),
						],
						$include_locations
					);
					?>
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('Do Not Display On', 'hnice'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
					   title="<?php echo esc_attr__('Add locations for where this template should not appear.', 'hnice'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
						...
						[
							'title'          => esc_html__('Exclude On', 'hnice'),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => esc_html__('Add Exclusion Rule', 'hnice'),
							'rule_type'      => 'exclude',
						],
						$exclude_locations
					);
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e('User Roles', 'hnice'); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__('Display custom template based on user role.', 'hnice'); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					HFE\Lib\Astra_Target_Rules_Fields::target_user_role_settings_field(
						...
						[
							'title'          => esc_html__('Users', 'hnice'),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => esc_html__('Add User Rule', 'hnice'),
						],
						$users
					);
					?>
	...
	public static function get_template_id() {
		...
		$hfe_templates = Astra_Target_Rules_Fields::get_instance()->get_posts_by_conditions('hnice-breadcrumb', $option);
	...
	public function block_template_frontend() {
		if (is_singular('hnice-breadcrumb') && !current_user_can('edit_posts')) {
	...
	function load_canvas_template($single_template) {
		...
		if ('hnice-breadcrumb' == $post->post_type) {
