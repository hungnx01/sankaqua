    ...
    function quick_product_variable() {
        ...
        $(document).on('click', btnSelector, function (e) {
            ...
            if ($product.hasClass('quick-shop-loaded')) {
                ...
                $('body').trigger('hnice-quick-view-displayed');
                return;
            }
            ...
            $.ajax({
                url: hniceAjax.ajaxurl, data: {
                    action: 'hnice_quick_shop', id: id,
            ...
            // Ajax action.
            xhr = $.ajax({
                url: wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'hnice_add_to_cart'),
                data: formData,
                type: 'POST',
                processData: false,
                contentType: false,
    ...
    function sendRequest(url) {
        ...
        xhr = $.ajax({
            type: "GET", url: url, beforeSend: function () {
                var $products = $('ul.hnice-products');
                $products.addClass('preloader');
            }, success: function (data) {
                let $html = $(data);
                $('#main ul.hnice-products').replaceWith($html.find('#main ul.hnice-products'));
                $('#main .woocommerce-pagination').replaceWith($html.find('#main .woocommerce-pagination'));
                window.history.pushState(null, null, url);
                xhr = false;
                $(document).trigger('hnice-products-loaded')
            }
        });
    }

    $body.on('change', '.hnice-products-per-page #per_page', function (e) {
        e.preventDefault();
        var url = this.value;
        sendRequest(url);
    });
