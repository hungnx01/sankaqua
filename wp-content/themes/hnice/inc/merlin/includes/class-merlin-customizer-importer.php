...
class Merlin_Customizer_Importer {
	...
	public static function import( $customizer_import_file_path ) {
		...
		Merlin_Logger::get_instance()->info( esc_html__( 'The customizer import has finished successfully', 'hnice' ) );
		return true;
	}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'hnice' ),
					$import_file_path
				)
			);
		}
		...
		if ( empty( $raw ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file does not have any content in it. Please make sure to use the correct customizer import file.', 'hnice' )
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'hnice' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'hnice' )
			);
		}
