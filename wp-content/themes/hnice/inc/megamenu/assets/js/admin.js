( function($) {
	var Hnice_Modal_Menu_Item = Backbone.View.extend({

		initialize: function( data ) {
			this.model = new Backbone.Model( data );
			this.template = '<span class="hnice-edit-menu"><i class="fa fa-pencil"></i> Mega</span>';
		...
		_openModalHandler: function() {
			window.hnice_menu_modal = new Hnice_Modal( this.model );
		}

	});

	// Backbone Modal View
	var Hnice_Modal = Backbone.View.extend({
		...
		initialize: function( model ) {
			...
			this.events = {
				...
				'click .hnice-modal-overlay' : '_closeHandler',
		...
		render: function() {
			var that = this;
			this.template = $( '#tpl-hnice-menu-item-modal' ).html();
			...
			data.action = 'hnice_load_menu_data';
			data.nonce = hnice_memgamnu_params.nonces.load_menu_data;
			...
			if ( this.model.get( 'is_loading' ) === true ) {
				$.ajax({
					type: 'POST',
					url: hnice_memgamnu_params.ajaxurl,
					data: data,
					beforeSend: function() {
		...
		_submitEditMenuForm: function( e ) {
			...
			data.push({
				name: 'action',
				value: 'hnice_update_menu_item_data'
			});
			...
			$.ajax({
				...
				data: data,
				beforeSend: function() {
					form.find( '.hnice-modal-footer button' ).attr( 'disabled', true );
					form.find( 'button[type="submit"]' ).prepend( '<i class="fa fa-spin fa-spinner"></i> ' );
				}
			}).always( function() {
				form.find( '.hnice-modal-footer button' ).attr( 'disabled', false );
			} ).done( function( res ) {
				if ( res.status === true ) {
					that.model.destroy();
				}
	...
	$( document ).on( 'mouseenter', '#menu-to-edit .menu-item-handle', function(e) {
		...
		menu_title.append( new Hnice_Modal_Menu_Item({
			menu_id: menu_id,
			is_loading: true,
			istop:istop?1:0
		}).render().el );
		...
		var menu_item = $( this );
		var menu_title = menu_item.find( '.item-title' );
		menu_title.find( '.hnice-edit-menu' ).remove();
	} );
} )(jQuery);
