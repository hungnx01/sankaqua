...
class WXRImporter extends \WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', esc_html__( 'Could not open the file for parsing', 'hnice' ) );
		}
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'hnice' ),
							$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', esc_html__( 'The file does not exist, please try again.', 'hnice' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( esc_html__( 'Invalid author mapping', 'hnice' ) );
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'hnice' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'hnice' ),
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice( sprintf(
					__( 'Skipping attachment "%s", fetching attachments disabled', 'hnice' ),
					$data['post_title']
				) );
				return false;
			}
		...
		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'hnice' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'hnice' ),
			$original_id,
			$post_id
		) );
					...
					if ( 'post_format' === $taxonomy ) {
						...
						if ( empty( $term_id ) ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								$this->logger->warning( sprintf(
									esc_html__( 'Failed to import term: %s - %s', 'hnice' ),
									esc_html( $taxonomy ),
									esc_html( $term['name'] )
								) );
								continue;
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', esc_html__( 'Invalid file type', 'hnice' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"', 'hnice' ),
				$userdata['user_login']
			) );
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"', 'hnice' ),
			$userdata['user_login']
		) );
		$this->logger->debug( sprintf(
			__( 'User %d remapped to %d', 'hnice' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term_meta( $meta, $term_id, $term ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning( sprintf(
						__( 'Failed to add metakey: %s, metavalue: %s to term_id: %d', 'hnice' ),
						$key,
						$value,
						$term_id
					) );
					...
					$this->logger->debug( sprintf(
						__( 'Meta for term_id %d : %s => %s ; successfully added!', 'hnice' ),
						$term_id,
						$key,
						$value
					) );
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'hnice' ),
					$code,
		...
		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'hnice' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( esc_html__( 'Remote file is too large, limit is %s', 'hnice' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug( sprintf(
				...
				__( 'Running post-processing for post %d', 'hnice' ),
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the post parent for "%s" (post #%d)', 'hnice' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with parent %d, but could not be found', 'hnice' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for "%s" (post #%d)', 'hnice' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with author "%s", but could not be found', 'hnice' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.', 'hnice' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update "%s" (post #%d) with mapped data', 'hnice' ),
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning( sprintf(
				__( 'Could not find the menu object for "%s" (post #%d)', 'hnice' ),
				get_the_title( $post_id ),
				$post_id
			) );
			$this->logger->debug( sprintf(
				__( 'Post %d was imported with object "%d" of type "%s", but could not be found', 'hnice' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the comment parent for comment #%d', 'hnice' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with parent %d, but could not be found', 'hnice' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for comment #%d', 'hnice' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with author %d, but could not be found', 'hnice' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update comment #%d with mapped data', 'hnice' ),
					$comment_id
				) );
				continue;
			}
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if( empty( $termid ) || ! is_numeric( $termid ) ) {
				$this->logger->warning( sprintf(
					__( 'Faulty term_id provided in terms-to-be-remapped array %s', 'hnice' ),
					$termid
					) );
				continue;
			}
			...
			if( empty( $term_taxonomy ) ){
				$this->logger->warning( sprintf(
					__( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'hnice' ),
					$term_id
					) );
				continue;
			}
			...
			if ( empty( $parent_slug ) ) {
				$this->logger->warning( sprintf(
					__( 'No parent_slug identified in remapping-array for term: %d', 'hnice' ),
					$term_id
					) );
				continue;
			}
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				$this->logger->warning( sprintf(
					__( 'The term(%d)"s parent_slug (%s) is not found in the remapping-array.', 'hnice' ),
					$term_id,
					$parent_slug
					) );
				continue;
			...
			if ( empty( $termattributes ) ) {
				$this->logger->warning( sprintf(
					__( 'No data returned by get_term_by for term_id #%d', 'hnice' ),
					$term_id
					) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
			$this->logger->warning( sprintf(
					__( 'Could not update "%s" (term #%d) with mapped data', 'hnice' ),
					$termattributes['name'],
					$term_id
				) );
				$this->logger->debug( $result->get_error_message() );
			...
			delete_term_meta( $term_id, '_wxr_import_parent' );
			$this->logger->debug( sprintf(
				__( 'Term %d was successfully updated with parent %d', 'hnice' ),
				$term_id,
				$mapped_parent
			) );
		}
	...
	function remap_featured_images() {
		...
		$this->logger->info( esc_html__( 'Starting remapping of featured images', 'hnice' ) );
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->mapping['post'][ $value ] ) ) {
				...
				if ( $new_id !== $value ) {
					$this->logger->info( sprintf( esc_html__( 'Remapping featured image ID %d to new ID %d for post ID %d', 'hnice' ), $value, $new_id, $post_id ) );
