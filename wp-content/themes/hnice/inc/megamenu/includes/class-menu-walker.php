 ...
 */
class Hnice_Megamenu_Walker extends Walker_Nav_Menu {
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if (isset($item->mega_data) && !empty($item->mega_data)) {
            if (!empty($item->mega_data['icon'])) {
                ...
                $icon = hnice_megamenu_get_icon_html($icon_class, $item->mega_data);
            }
            if (!empty($item->mega_data['badge_title'])) {
                $badge = hnice_get_badge_html($item->mega_data['badge_title'], $item->mega_data);
            }
        }
    ...
    public function display_element($element, &$children_elements, $max_depth, $depth, $args, &$output) {
        if (!$element || !hnice_is_elementor_activated()) {
            return;
        }
        // echo '<pre>'.print_r( $element ,1 );die;
        $element->mega_data = hnice_megamenu_get_item_data($element->ID);
    ...
    public function render_megamenu_elementor($item, $args, $depth) {
        ...
        if (class_exists('Elementor\Plugin')) {
            $post_id = hnice_megamenu_get_post_related_menu($item->ID);
