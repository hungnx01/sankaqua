...
class Merlin_Widget_Importer {
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file could not be found.', 'hnice' )
			);
		}
		...
		if ( empty( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file does not have any content in it.', 'hnice' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import data could not be read. Please try a different file.', 'hnice' )
			);
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = esc_html__( 'Sidebar does not exist in theme (moving widget to Inactive)', 'hnice' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = esc_html__( 'Site does not support widget', 'hnice' ); // Explain why widget not imported.
				}
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = esc_html__( 'Widget already exists', 'hnice' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message      = esc_html__( 'Imported', 'hnice' );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = esc_html__( 'Imported to Inactive', 'hnice' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__( 'No Title', 'hnice' ); // Show "No Title" if widget instance is untitled.
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'hnice' );
		}
