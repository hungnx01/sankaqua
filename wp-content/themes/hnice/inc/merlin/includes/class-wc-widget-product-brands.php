 ...
 * @extends WC_Widget
 */
class Hnice_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'hnice');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'hnice');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'hnice'),
				'label' => esc_html__('Title', 'hnice'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'hnice'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'hnice'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'hnice'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'hnice'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'hnice'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'hnice'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'hnice'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Hnice_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'hnice');
			...
			echo '<ul class="product-brands" id="hnice-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Hnice_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'hnice'),
		'singular_name'              => esc_html__('Brands', 'hnice'),
		'menu_name'                  => esc_html__('Brands', 'hnice'),
		'all_items'                  => esc_html__('All Brands', 'hnice'),
		'parent_item'                => esc_html__('Parent Brand', 'hnice'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'hnice'),
		'new_item_name'              => esc_html__('New Brand Name', 'hnice'),
		'add_new_item'               => esc_html__('Add New Brands', 'hnice'),
		'edit_item'                  => esc_html__('Edit Brand', 'hnice'),
		'update_item'                => esc_html__('Update Brand', 'hnice'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'hnice'),
		'search_items'               => esc_html__('Search Brands', 'hnice'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'hnice'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'hnice'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
