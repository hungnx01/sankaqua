...
class Hnice_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce hnice_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Hnice custom)', 'hnice');
        $this->widget_id          = 'hnice_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Hnice Filter Products by Attribute', 'hnice');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'hnice'),
                'label' => esc_html__('Title', 'hnice'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'hnice'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'hnice'),
                'options' => array(
                    'and' => esc_html__('AND', 'hnice'),
                    'or'  => esc_html__('OR', 'hnice'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="hnice-button-type" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="hnice-color-type" href="' . esc_url($link) . '">';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="hnice-image-type" href="' . esc_url($link) . '">';
