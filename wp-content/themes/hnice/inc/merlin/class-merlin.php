...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
    ...
    public function admin_page() {
        ...
        $texts = array(
            'something_went_wrong' => esc_html__('Something went wrong. Please refresh the page and try again!', 'hnice'),
        );
    ...
    public function svg($args = array()) {
        ...
        if (empty($args)) {
            return esc_html__('Please define default parameters in the form of an array.', 'hnice');
        }

        // Define an icon.
        if (false === array_key_exists('icon', $args)) {
            return esc_html__('Please define an SVG icon filename.', 'hnice');
        }
    ...
    public function steps() {
        ...
        if (!empty($this->import_files)) {
            $this->steps['content'] = array(
                'name' => esc_html__('Content', 'hnice'),
                'view' => array($this, 'content'),
            );
        }

        $this->steps['ready'] = array(
            'name' => esc_html__('Ready', 'hnice'),
    ...
    protected function welcome() {
        ...
        <?php
        $this->logger->debug(esc_html__('The welcome step has been displayed', 'hnice'));
    }
    ...
    protected function license() {
        ...
        <?php
        $this->logger->debug(esc_html__('The license activation step has been displayed', 'hnice'));
    }
    ...
    protected function child() {
        ...
        <?php
        $this->logger->debug(esc_html__('The child theme installation step has been displayed', 'hnice'));
    }
    ...
    protected function plugins() {
                        ...
                        <?php foreach ($required_plugins as $slug => $plugin) : ?>
                            <li data-slug="<?php echo esc_attr($slug); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr($slug); ?>">
                                    ...
                                    <span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e('Required', 'hnice'); ?>">
										<?php esc_html_e('req', 'hnice'); ?>
									</span>
								</span>
                                </label>
                            </li>
        ...
        <?php
        $this->logger->debug(esc_html__('The plugin installation step has been displayed', 'hnice'));
    }
    ...
    protected function content() {
            ...
            <?php if (1 < count($this->import_files)) : ?>
                ...
                <div class="merlin__select-control-wrapper">
                    ...
                    <div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__('Select Demo', 'hnice'); ?>">
        ...
        <?php
        $this->logger->debug(esc_html__('The content import step has been displayed', 'hnice'));
    }
    ...
    protected function ready() {
        ...
        <?php
        $this->logger->debug(esc_html__('The final step has been displayed', 'hnice'));
    }
    ...
    public function generate_child() {
        ...
        if (!file_exists($path)) {
            ...
            $this->logger->debug(esc_html__('The existing child theme was activated', 'hnice'));
        ...
        $this->logger->debug(esc_html__('The newly generated child theme was activated', 'hnice'));
    ...
    public function _ajax_activate_license() {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce')) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Yikes! The theme activation failed. Please try again or contact support.', 'hnice'),
                )
            );
        }
        ...
        if (empty($_POST['license_key'])) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Please add your license key before attempting to activate one.', 'hnice'),
                )
            );
        }
        ...
        $this->logger->debug(esc_html__('The license activation was performed with the following results', 'hnice'), $result);
    ...
    protected function edd_activate_license($license) {
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (is_wp_error($response)) {
                ...
                $message = esc_html__('An error occurred, please try again.', 'hnice');
            }
        } else {
                ...
                switch ($license_data->error) {
                    ...
                    case 'expired':
                        ...
                        /* translators: Expiration date */
                            esc_html__('Your license key expired on %s.', 'hnice'),
                            date_i18n(get_option('date_format'), strtotime($license_data->expires, current_time('timestamp')))
                        );
                        break;

                    case 'revoked':
                        $message = esc_html__('Your license key has been disabled.', 'hnice');
                        break;

                    case 'missing':
                        $message = esc_html__('This appears to be an invalid license key. Please try again or contact support.', 'hnice');
                        break;

                    case 'invalid':
                    case 'site_inactive':
                        $message = esc_html__('Your license is not active for this URL.', 'hnice');
                        break;

                    case 'item_name_mismatch':
                        /* translators: EDD Item Name */
                        $message = sprintf(esc_html__('This appears to be an invalid license key for %s.', 'hnice'), $this->edd_item_name);
                        break;

                    case 'no_activations_left':
                        $message = esc_html__('Your license key has reached its activation limit.', 'hnice');
                        break;
    ...
    public function generate_child_functions_php($slug) {
        ...
        $this->logger->debug(esc_html__('The child theme functions.php content was generated', 'hnice'));
    ...
    public function generate_child_style_css($slug, $parent, $author, $version) {
        ...
        $this->logger->debug(esc_html__('The child theme style.css content was generated', 'hnice'));
    ...
    public function generate_child_screenshot($path) {
        ...
        if (!empty($screenshot) && file_exists($screenshot)) {
            ...
            $this->logger->debug(esc_html__('The child theme screenshot was copied to the child theme, with the following result', 'hnice'), array('copied' => $copied));
        } else {
            $this->logger->debug(esc_html__('The child theme screenshot was not generated, because of these results', 'hnice'), array('screenshot' => $screenshot));
        }
    }
    ...
    function _ajax_plugins() {
        ...
        foreach ($plugins['activate'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Activating', 'hnice'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['update'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Updating', 'hnice'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['install'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2'       => -1,
                    'message'       => esc_html__('Installing', 'hnice'),
                );
                break;
            }
        }
        ...
        if ($json) {
            $this->logger->debug(
                esc_html__('A plugin with the following data will be processed', 'hnice'),
                array(
            ...
            $json['message'] = esc_html__('Installing', 'hnice');
            wp_send_json($json);
        } else {
            $this->logger->debug(
                esc_html__('A plugin with the following data was processed', 'hnice'),
                array(
                    'plugin_slug' => $_POST['slug'],
                )
            );
            ...
            wp_send_json(
                array(
                    'done'    => 1,
                    'message' => esc_html__('Success', 'hnice'),
                )
            );
        }
    ...
    function _ajax_content() {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])) {
            $this->logger->error(esc_html__('The content importer AJAX call failed to start, because of incorrect data', 'hnice'));

            wp_send_json_error(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Invalid content!', 'hnice'),
                )
            );
        }
        ...
        if (isset($_POST['proceed'])) {
            if (is_callable($this_content['install_callback'])) {
                $this->logger->info(
                    esc_html__('The content import AJAX call will be executed with this import data', 'hnice'),
                    array(
        ...
        if ($json) {
            ...
            $this->logger->error(
                esc_html__('The content import AJAX call failed with this passed data', 'hnice'),
                array(
            ...
            wp_send_json(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Error', 'hnice'),
                    'logs'    => '',
                    'errors'  => '',
                )
            );
    ...
    public function _ajax_get_total_content_import_items() {
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') && empty($_POST['selected_index'])) {
            $this->logger->error(esc_html__('The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'hnice'));

            wp_send_json_error(
                array(
                    'error'   => 1,
                    'message' => esc_html__('Invalid data!', 'hnice'),
                )
            );
        }
    ...
    protected function get_import_data($selected_import_index = 0) {
        ...
        if (!empty($import_files['content'])) {
            $content['content'] = array(
                'title'            => esc_html__('Content', 'hnice'),
                'description'      => esc_html__('Demo content data.', 'hnice'),
                'pending'          => esc_html__('Pending', 'hnice'),
                'installing'       => esc_html__('Installing', 'hnice'),
        ...
        if (!empty($import_files['widgets'])) {
            $content['widgets'] = array(
                'title'            => esc_html__('Widgets', 'hnice'),
                'description'      => esc_html__('Sample widgets data.', 'hnice'),
                'pending'          => esc_html__('Pending', 'hnice'),
                'installing'       => esc_html__('Installing', 'hnice'),
                'success'          => esc_html__('Success', 'hnice'),
        ...
        if (!empty($import_files['sliders'])) {
            $content['sliders'] = array(
                'title'            => esc_html__('Revolution Slider', 'hnice'),
                'description'      => esc_html__('Sample Revolution sliders data.', 'hnice'),
                'pending'          => esc_html__('Pending', 'hnice'),
                'installing'       => esc_html__('Installing', 'hnice'),
                'success'          => esc_html__('Success', 'hnice'),
        ...
        if (!empty($import_files['options'])) {
            $content['options'] = array(
                'title'            => esc_html__('Home Page', 'hnice'),
                'description'      => esc_html__('Install homepage xml.', 'hnice'),
                'pending'          => esc_html__('Pending', 'hnice'),
                'installing'       => esc_html__('Installing', 'hnice'),
                'success'          => esc_html__('Success', 'hnice'),
                'install_callback' => array($this->importer, 'import'),
        ...
        if (!empty($import_files['redux'])) {
            $content['redux'] = array(
                'title'            => esc_html__('Redux Options', 'hnice'),
                'description'      => esc_html__('Redux framework options.', 'hnice'),
                'pending'          => esc_html__('Pending', 'hnice'),
                'installing'       => esc_html__('Installing', 'hnice'),
                'success'          => esc_html__('Success', 'hnice'),
        ...
        if (false !== has_action('merlin_after_all_import')) {
            $content['after_import'] = array(
                'title'            => esc_html__('After import setup', 'hnice'),
                'description'      => esc_html__('After import setup.', 'hnice'),
                'pending'          => esc_html__('Pending', 'hnice'),
                'installing'       => esc_html__('Installing', 'hnice'),
                'success'          => esc_html__('Success', 'hnice'),
...
//        $response = $importer->importSliderFromPost(true, true, $file);
        ...
        $this->logger->info(esc_html__('The revolution slider import was executed', 'hnice'));
    ...
    public function pt_importer_new_ajax_request_response_data($data) {
        ...
        $data['message']  = esc_html__('Installing', 'hnice');
        $data['proceed']  = 'true';
    ...
    public function after_content_import_setup() {
        ...
        if ($homepage) {
            ...
            $this->logger->debug(esc_html__('The home page was set', 'hnice'), array('homepage_id' => $homepage));
        }
        ...
        if ($blogpage) {
            ...
            $this->logger->debug(esc_html__('The blog page was set', 'hnice'), array('blog_page_id' => $blogpage));
        }
    }
    ...
    public function before_content_import_setup() {
        ...
        if (!empty($hello_world)) {
            ...
            $this->logger->debug(esc_html__('The Hello world post status was set to draft', 'hnice'));
        }
    }
    ...
    public function validate_import_file_info($import_files) {
        ...
        foreach ($import_files as $import_file) {
            if (!empty($import_file['import_file_name'])) {
                ...
                $this->logger->warning(esc_html__('This predefined demo import does not have the name parameter: import_file_name', 'hnice'), $import_file);
            }
        }
