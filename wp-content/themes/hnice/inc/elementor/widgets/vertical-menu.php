...
class Hnice_Elementor_Vertiacl_Menu extends Elementor\Widget_Base
{
    ...
    {
        return 'hnice-vertical-menu';
    }

    public function get_title()
    {
        return esc_html__('Hnice Vertical Menu', 'hnice');
    }
    ...
    protected function register_controls()
    {
        ...
        if (!empty($menus)) {
            $this->add_control(
                'menu',
                [
                    'label' => esc_html__('Menu', 'hnice'),
                    'type' => Controls_Manager::SELECT,
                    'options' => $menus,
                    'default' => array_keys($menus)[0],
                    'save_default' => true,
                    'separator' => 'after',
                    'description' => sprintf(__('Go to the <a href="%s" target="_blank">Menus screen</a> to manage your menus.', 'hnice'), admin_url('nav-menus.php')),
                ]
            );
        } else {
            $this->add_control(
                ...
                [
                    ...
                    'raw' => '<strong>' . esc_html__('There are no menus in your site.', 'hnice') . '</strong><br>' . sprintf(__('Go to the <a href="%s" target="_blank">Menus screen</a> to create one.', 'hnice'), admin_url('nav-menus.php?action=edit&menu=0')),
                    'separator' => 'after',
        ...
        $this->add_control(
            'nav_vertiacl_layout',
            [
                'label' => esc_html__('Menu Layout', 'hnice'),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'style-1' => esc_html__('Dropdown', 'hnice'),
                    'style-2' => esc_html__('Navbar', 'hnice'),
                ],
                'default' => 'style-1',
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    ...
                    '{{WRAPPER}} .vertical-navigation .vertical-navigation-header .hnice-icon' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                'label' => esc_html__('Color Icon Hover', 'hnice'),
                'type' => Controls_Manager::COLOR,
                'default' => '',
                'selectors' => [
                    ...
                    '{{WRAPPER}} .vertical-navigation:hover .vertical-navigation-header .hnice-icon' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->add_control('vertical_menu_separator', [
            'label' => esc_html__('Separator', 'hnice'),
        ...
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Sub Menu', 'hnice'),
                'tab' => Controls_Manager::TAB_STYLE,
            ]
        );
        $this->add_group_control(
        ...
        $this->add_control(
            ...
            [
                'label' => esc_html__('Color Active', 'hnice'),
                'type' => Controls_Manager::COLOR,
                'default' => '',
                'selectors' => [
    ...
    protected function render()
    {
        ...
        <div <?php $this->print_render_attribute_string('wrapper'); ?>>
            <nav class="vertical-navigation" aria-label="<?php esc_attr_e('Vertiacl Navigation', 'hnice'); ?>">
...
$widgets_manager->register(new Hnice_Elementor_Vertiacl_Menu());
