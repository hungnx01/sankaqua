...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			$this->logger->critical( esc_html__( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'hnice' ) );
		...
		if ( ! $status ) {
			$this->logger->error( esc_html__( 'Could not open the XML file for parsing!', 'hnice' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->error( esc_html__( 'Content import start error: ', 'hnice' ) . $result->get_error_message() );
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
			...
			$this->logger->info( esc_html__( 'New AJAX call!', 'hnice' ) );
