...
class Merlin_Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		Merlin_Logger::get_instance()->error( esc_html__( 'The file was not able to save to disk, while trying to download it', 'hnice' ), array( 'url' => $url, 'filename' => $filename ) );
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'hnice' )
			);
		}
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'hnice' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
