(function ($) {
    ...
    $.aroageCheck = function (options) {
        ...
        const _this = {
            ...
            errors: [],
            setValues() {
                const month = $(".hnice-av .month").val();
                const day = $(".hnice-av .day").val();
                _this.month = month;
                _this.day = day.replace(/^0+/, ""); // remove leading zero
                _this.year = $(".hnice-av .year").val();
            },
            validate() {
                _this.errors = [];
            ...
            buildHtml() {
                ...
                html += "<div class=\"hnice-av-overlay\"></div>";
                html += "<div class=\"hnice-av\" style=\"" + style + " \">";
                ...
                if (settings.description !== "") {
                    html += `<p class="hnice-av-description">${settings.description}</p>`;
                }
                html += `<p class="hnice-av-copy">${copy.replace("[age]", `<strong>${settings.minAge}</strong>`)}`;
                +`</p>`;
                ...
                $(".hnice-av-overlay").animate({
                    opacity: 1,
                }, 500, () => {
                    _this.reCenter($(".hnice-av"));
                    $(".hnice-av").css({
                        opacity: 1,
                    });
                });

                $(".hnice-av .day, .hnice-av .year").focus(function () {
                    ...
                    $(".hnice-av").animate({
                        ...
                        $(".hnice-av-overlay").animate({
                            ...
                            if (settings.redirectTo !== "") {
                                ...
                                $(".hnice-av-overlay, .hnice-av").remove();
                            }
                        });
                    });
                }, settings.messageTime);
